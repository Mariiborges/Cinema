...
public static class IngressoApi
{
    public static void ConfigureIngressoApi(this WebApplication app)
    {
        ...
        {
            // Verificar se já existe um ingresso com o mesmo assento e sessão
            var ingressoExistente = await db.Ingressos
                .FirstOrDefaultAsync(i => i.assento == ingresso.assento && i.idSessao == ingresso.idSessao);

            if (ingressoExistente != null)
            {
                return Results.Conflict("Já existe um ingresso para esse assento e sessão.");
            }
